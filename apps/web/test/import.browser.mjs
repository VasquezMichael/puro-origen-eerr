...
try {for(const [width,height] of [[1440,900],[1280,720],[1024,768],[768,1024],[390,844]]) {
 ...
 const zip=await JSZip.loadAsync(templates.xlsx),sheetPath='xl/worksheets/sheet2.xml',xml=await zip.file(sheetPath).async('string');zip.file(sheetPath,xml.replace(/<c r="F2"[^>]*>.*?<\/c>/,'<c r="F2"><v>1500</v></c>'));templates.numericXlsx=await zip.generateAsync({type:'nodebuffer'});
 ...
 if(req.method()==='POST') {await new Promise(r=>setTimeout(r,300));assert.match(req.postDataBuffer().toString(),/filename="carga.(?:csv|xlsx)"/);
 if(path.endsWith('/import/preview')){const plan=importPlan(row.structure,[{row:2,code:nodes[3].code,amount:invalid?'':'20+5',quantity:'3',...(invalid?{amountIssue:'El importe debe estar guardado como texto en Excel. Cambiá el formato de la celda a Texto y volvé a ingresar el valor.'}:{})}]);delete plan.changes;body={...plan,destination:{id,branchId,branchName:'Sucursal ficticia',year:2026,month:9},fileName:invalid?'carga.xlsx':'carga.csv',format:invalid?'xlsx':'csv',warnings:['Las notas y la estructura se conservan.'],revision:row.revision,structuralRevision:1,previewToken:invalid?null:'fixture-preview',expiresAt:'2026-09-23T12:05:00Z'};}
 ...
 await page.getByRole('button',{name:'Importar archivo',exact:true}).click();await dialog.getByLabel('Archivo CSV o XLSX').setInputFiles({name:'carga.xlsx',mimeType:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',buffer:templates.numericXlsx});
 await dialog.getByRole('button',{name:'Generar vista previa'}).click();await dialog.getByText('No se puede confirmar: 1 errores').waitFor();await dialog.getByText('Cambiá el formato de la celda a Texto',{exact:false}).scrollIntoViewIfNeeded();assert.match(await dialog.innerText(),/Fila 2 · importe_o_expresion/);assert.match(await dialog.innerText(),/Seleccionado: carga.xlsx/);assert.ok(await dialog.getByRole('button',{name:'Confirmar importación'}).isDisabled());assert.equal(writes,0);await shot('invalid');assert.equal(await page.evaluate(()=>{const event=new Event('beforeunload',{cancelable:true});window.dispatchEvent(event);return event.defaultPrevented;}),true);
 const chooser=page.waitForEvent('filechooser');await dialog.getByRole('button',{name:'Reemplazar archivo'}).click();await (await chooser).setFiles({name:'carga.csv',mimeType:'text/csv',buffer:templates.csv});assert.equal(await dialog.getByText('No se puede confirmar: 1 errores').count(),0);assert.match(await dialog.innerText(),/Seleccionado: carga.csv/);
